...
static void handle_events(Game *game)
{
    ...
    while (SDL_PollEvent(&event))
    {
        ...
        case SDL_KEYDOWN:
            ...
            if (event.key.keysym.sym == SDLK_RETURN && game->state == STATE_MENU)
            {
            ...
            if (event.key.keysym.sym == SDLK_r && game->state == STATE_GAME_OVER)
            {
                /* Reinitialiser tout */
                game->player.score = 0;
                game->player.lives = 3;
            ...
            if (event.key.keysym.sym == SDLK_RETURN && game->state == STATE_LEVEL_UP)
            {
                game->state = STATE_PLAYING;
...
static void render(Game *game)
{
    ...
    case STATE_MENU:
        ...
        SDL_RenderClear(game->renderer);
        /* Le texte du menu sera dans menu.c */
        break;
    case STATE_PLAYING:
        /* Fond d'ecran */
        if (game->background)
        ...
        /* Dessiner le viseur */
        {
            int x = game->player.mouse_x;
            int y = game->player.mouse_y;
            if (game->crosshair)
            {
                SDL_Rect cr = {x - 20, y - 20, 40, 40};
                SDL_RenderCopy(game->renderer, game->crosshair, NULL, &cr);
            }
            else
            {
                /* Viseur dessine en SDL2 */
                SDL_SetRenderDrawColor(game->renderer, 255, 50, 50, 255);
                SDL_RenderDrawLine(game->renderer, x - 18, y, x + 18, y);
                SDL_RenderDrawLine(game->renderer, x, y - 18, x, y + 18);
            }
        }
...
void cleanup(Game *game)
{
    /* Sons */
    if (game->sound_shoot)
        Mix_FreeChunk(game->sound_shoot);
    if (game->sound_dead)
        Mix_FreeChunk(game->sound_dead);
    if (game->music)
        Mix_FreeMusic(game->music);
    /* Textures */
    if (game->background)
        SDL_DestroyTexture(game->background);
    if (game->bird_frames[0])
        SDL_DestroyTexture(game->bird_frames[0]);
    if (game->bird_frames[1])
        SDL_DestroyTexture(game->bird_frames[1]);
    if (game->bird_dead)
        SDL_DestroyTexture(game->bird_dead);
    if (game->crosshair)
        SDL_DestroyTexture(game->crosshair);
    /* Police */
    if (game->font)
